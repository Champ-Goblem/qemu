                                    ...
                                    uint32_t dev_tree_size, const char *CommandLine)
{
    ...
    boot_args->topOfKernelData = top_of_kernel_data;
...
uint64_t arm_load_macho(struct arm_boot_info *info, uint64_t *pentry, AddressSpace *as)
{
    ...
    uint64_t low_addr_temp, high_addr_temp, virt_base = 0, pc = 0;

    xnu_arm_macho_highest_lowest(mh, &low_addr_temp, &high_addr_temp);
    uint64_t kern_size = high_addr_temp - low_addr_temp;
    ...
    hwaddr kern_base_addr = VAtoPA(low_addr_temp);
    rom_add_blob_fixed_as("kernel", rom_buf, kern_size, kern_base_addr, as);
    ret = true;
    fprintf(stderr, "low: %lx high: %lx kern_entry: %lx\n", low_addr_temp, high_addr_temp, pc);
    fprintf(stderr, "kern_base: %lx\n", *(uint64_t*)kern_base_addr);
    ...
    xnu_arm_setup_boot_args(boot_args, (mem_base + kernel_load_offset), mem_base, info->ram_size, 
                            VAtoPA(high_addr_temp), NULL, 0, 0, info->kernel_cmdline);
